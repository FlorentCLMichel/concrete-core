...
use concrete_core::prelude::{
    ...
    LweSeededToLweCiphertextTransmutationEngine, PlaintextEntity,
};

/// A fixture for the types implementing the `LweSeededToLweCiphertextTransmutationEngine` trait.
pub struct LweSeededToLweCiphertextTransmutationFixture;

#[derive(Debug)]
pub struct LweSeededToLweCiphertextTransmutationParameters {
    pub noise: Variance,
...
impl<Precision, Engine, Plaintext, SecretKey, InputCiphertext, OutputCiphertext>
    ...
    for LweSeededToLweCiphertextTransmutationFixture
where
    Precision: IntegerPrecision,
    Engine: LweSeededToLweCiphertextTransmutationEngine<InputCiphertext, OutputCiphertext>,
...
{
    type Parameters = LweSeededToLweCiphertextTransmutationParameters;
    ...
    fn generate_parameters_iterator() -> Box<dyn Iterator<Item = Self::Parameters>> {
        Box::new(
            vec![
                LweSeededToLweCiphertextTransmutationParameters {
    ...
    fn execute_engine(
        ...
        let ciphertext = unsafe {
            engine.transmute_lwe_seeded_ciphertext_to_lwe_ciphertext_unchecked(seeded_ciphertext)
        };
        (ciphertext,)
    }
