...
use crate::specification::engines::{
    LweSeededToLweCiphertextTransmutationEngine, LweSeededToLweCiphertextTransmutationEngineError,
};
use crate::specification::entities::LweSeededCiphertextEntity;

/// # Description:
/// Implementation of [`LweSeededToLweCiphertextTransmutationEngine`] for [`DefaultEngine`] that
/// operates on 32 bits integers.
impl LweSeededToLweCiphertextTransmutationEngine<LweSeededCiphertext32, LweCiphertext32>
    for DefaultEngine
{
    /// # Example:
    /// ```
    ...
    /// let ciphertext = engine.transmute_lwe_seeded_ciphertext_to_lwe_ciphertext(seeded_ciphertext)?;
    /// #
    ...
    ) -> Result<LweCiphertext32, LweSeededToLweCiphertextTransmutationEngineError<Self::EngineError>>
    {
        Ok(unsafe {
            self.transmute_lwe_seeded_ciphertext_to_lwe_ciphertext_unchecked(lwe_seeded_ciphertext)
        })
    }
    ...
    unsafe fn transmute_lwe_seeded_ciphertext_to_lwe_ciphertext_unchecked(
        ...
        lwe_seeded_ciphertext.0.expand_into(&mut output_ciphertext);
...
impl LweSeededToLweCiphertextTransmutationEngine<LweSeededCiphertext64, LweCiphertext64>
    for DefaultEngine
{
    /// # Example:
    /// ```
    ...
    ) -> Result<LweCiphertext64, LweSeededToLweCiphertextTransmutationEngineError<Self::EngineError>>
    {
        Ok(unsafe {
            self.transmute_lwe_seeded_ciphertext_to_lwe_ciphertext_unchecked(lwe_seeded_ciphertext)
        })
