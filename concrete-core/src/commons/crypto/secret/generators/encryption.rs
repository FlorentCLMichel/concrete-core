...
impl<G: ByteRandomGenerator> EncryptionRandomGenerator<G> {
    ...
    /// Returns the number of bytes that have been generated for the mask so far if it fits in a
    /// usize. On a fresh generator this returns 0.
    pub fn generated_bytes_so_far(&self) -> Option<usize> {
        self.mask.generated_bytes_so_far()
    }
