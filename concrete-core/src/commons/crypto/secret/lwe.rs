    ...
    Kind: KeyKind,
{
    ...
    /// secret_key.encrypt_seeded_lwe(&mut encrypted, &plain, noise, &mut seeder);
    ///
    /// let mut encrypted_expanded = LweCiphertext::allocate(0u32, LweSize(257));
    /// encrypted.expand_into(&mut encrypted_expanded);
    ///
    /// let mut decrypted = Plaintext(0u32);
        ...
        Scalar: UnsignedTorus,
    {
        ...
        // TODO: find a way to instantiate an efficient EncryptionRandomGenerator potentially
        // provided by an engine. This is ok for the current DefaultEngine as it's supposed
