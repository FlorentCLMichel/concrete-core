...
pub trait RandomGenerator: Iterator<Item = u8> {
    ...
    /// Returns the number of bytes that have been generated so far. On a fresh generator this
    /// returns 0.
    ///
    /// Note:
    /// -----
    ...
    /// to encode such a large number. Consequently [`ByteCount`] uses the largest integer type
    /// available to encode this value: the `u128` type. For this reason, this method does not
    /// effectively return the number of generated bytes, but instead
    /// `min(2¹²⁸-1, generated_bytes)`.
...
pub mod generator_generic_test {
    ...
    pub fn test_fork_children<G: RandomGenerator>() {
        for _ in 0..REPEATS {
            ...
            assert_eq!(bounded.generated_bytes_so_far(), ByteCount(0));
            ...
            assert_eq!(
                bounded.generated_bytes_so_far(),
                ByteCount(n_bytes.0 as u128)
            );
