...
impl<BlockCipher: AesBlockCipher> AesCtrGenerator<BlockCipher> {
    ...
    /// Computes the number of bytes that have been generated so far. On a fresh generator this
    /// returns 0.
    ///
    /// Note :
    /// ------
    ///
    /// Note that `ByteCount` uses the `u128` datatype to store the byte count. Unfortunately, the
    /// number of generated bytes is in ⟦0;2¹³² -1⟧. When the number is greater than 2¹²⁸ - 1,
    /// we saturate the count at 2¹²⁸ - 1.
    pub fn generated_bytes_so_far(&self) -> ByteCount {
        TableIndex::distance(&self.state.table_index(), &TableIndex::FIRST).unwrap()
    }
